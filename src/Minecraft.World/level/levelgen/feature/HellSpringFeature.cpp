...
HellSpringFeature::HellSpringFeature(Block* block, bool forcePlacement) : Feature(false), mBlock(block), mIsForcePlacement(forcePlacement) {}
...
bool HellSpringFeature::place(Level* level, Random& random, const BlockPos& pos) {
    if (level->getBlockState(pos.above())->getBlock() == Blocks::NETHERRACK &&
        (level->getBlockState(pos)->getBlock()->getMaterial() == (Material*)Material::AIR || level->getBlockState(pos)->getBlock() == Blocks::NETHERRACK)) {
