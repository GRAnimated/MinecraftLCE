...
#include "net/minecraft/world/Explosion.h"
...
int dword_7100DFC814[6] = {1, -1, 0, 0, 0, 0};
int dword_7100DFC82C[6] = {0, 0, 1, -1, 0, 0};
int dword_7100DFC844[6] = {0, 0, 0, 0, 1, -1};
...
TextureAtlasSprite* Block::getTexture(LevelSource* levelSource, const BlockPos& pos,
    ...
    bool wasLeaves;
    for (int i = 0; i < 6; i++) {
        const BlockState* neighbourState = levelSource->getBlockState(
            pos.offset(dword_7100DFC814[i], dword_7100DFC82C[i], dword_7100DFC844[i]));
    ...
    Blocks::LEAVES->setFancy(0);
    Blocks::LEAVES2->setFancy(0);
    Blocks::LEAVES->setFancy(1);
    Blocks::LEAVES2->setFancy(1);
    return this->getTexture(direction, state);
}
